...
aapp.put("/people/:index", (req, res) => {
  const index = parseInt(req.params.index); // Convertir el índice en un número entero

  if (index >= 0 && index < people.length) {
    // Verificar que el índice esté dentro de los límites del array
    people[index] = req.body; // Actualizar el elemento en el índice indicado con los datos proporcionados en el cuerpo de la solicitud
    res.json(people[index]); // Responder con el objeto actualizado
  } else {
...
app.delete("/people/:index", (req, res) => {
  ...
  if (index >= 0 && index < people.length) {
    // Verificar que el índice esté dentro de los límites del array
    const deletedPerson = people.splice(index, 1); // Eliminar el elemento en el índice indicado
    res.json(deletedPerson[0]); // Responder con el objeto eliminado
  } else {
    res.status(404).json({ error: "Índice no válido" });
  }
});
